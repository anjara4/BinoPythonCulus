from PyQt5.QtWidgets import QWidget, QTabWidget, QTableView, QDesktopWidget, QFileDialog, QHeaderView
from PyQt5.QtWidgets import QSizePolicy, QLabel, QGroupBox, QLineEdit, QPushButton, QHBoxLayout, QVBoxLayout
...
class UI_main_configuration(QWidget):
    def __init__(self, selected_config):
        ...
        self.__table_list_config.horizontalHeader().setStretchLastSection(True)
        self.__table_list_config.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch)
        self.__table_list_config.setEditTriggers(QTableView.EditTrigger.NoEditTriggers)
        self.__table_list_config.setSelectionBehavior(QTableView.SelectionBehavior.SelectRows)
    ...
    def connect_config(self):
        if self.check_if_row_is_selectioned(self.__table_list_config.selectedIndexes()) and self.get_id_from_selected_row(nameConf=0)!='NameConf':
        ...
        elif self.get_id_from_selected_row(nameConf=0)=='NameConf' :
...
class Selected_config(QWidget):
    def __init__(self):
        ...
        lb_name_config = QLabel("Configuration name:")
...
class Select_config(QWidget):
    def __init__(self, table_list_config):
        super().__init__()
