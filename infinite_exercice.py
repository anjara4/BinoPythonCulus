...
class Infinite(QWidget):
    ...
    def __update_position(self):
        ...
        tmp = 55 * sin / (1 + cos * cos) #55 is a define size

        if self.get_is_object_vertical():
            self.__x = cos * (tmp / self.__diviser) * self.__x_scaling_vertical
            self.__x = self.__x + self.__display_width / 2 

            self.__y = tmp * self.__y_scaling_vertical
            self.__y = self.__y + self.__display_height / 2 
                
        else:
            self.__x = tmp * self.__x_scaling_horizontal 
            self.__x = self.__x + self.__display_width / 2 

            self.__y = cos * (tmp / self.__diviser) * self.__y_scaling_horizontal 
            self.__y = self.__y + self.__display_height / 2 

    def calculate_speed(self):
        self.__averageAngleSpeed = self.__two_pi * self.__speed / (5.24412 * self.__size / 2)
        self.__averageAngleSpeed = self.__averageAngleSpeed * (5.24412 / (1.24412 / self.__diviser + 4))
